...
| 风险 | 行覆盖率 | 分支覆盖率 | 单测通过率 | 契约通过率 | Spec 规则覆盖率 |
|------|----------|------------|------------|------------|------------------|
...
> 契约测试先行（验证接口契约），单测次之（验证业务语义），业务代码最后（带 @Spec 注解）。

| 阶段 | 内容 | 用例数 | 建议工期 |
|------|------|--------|----------|
| 1. 契约测试编写 | CT-xx 骨架 + contract mock 桩 | {{contract_test_count}} | {{contract_schedule}} |
| 2. 单测编写 | UT-xx 骨架 + Mock 依赖 | {{unit_test_count}} | {{unit_schedule}} |
